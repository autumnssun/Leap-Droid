...
class IMU
{
	...
	public:
		...
		typedef struct data{
			vector g_output,a_output,m_output;
		}data;
		...
		enum range_t
		{
			...
			L3G4200D_RANGE_2K = 0x20,
			L3G4200D_RANGE_500= 0x10,
			L3G4200D_RANGE_250= 0x00,
			L3G4200D_RANGE_ME= 0x80,
		};
		
		void start(bool _debug, float _alpha,float _beta,int _z_sample_rate);
		void readsensors(void);
		int zeroRateSample;
		data output;
		vector accel,gyro,magn,combine;				//reading from the sensors
	...
	private:
		...
		float alpha,beta;
		float gyro_scalefactor;
		vector z_accel_v,z_gyro_v,z_magn_v; //ground value of sensor, those are set when the sensors start up.
		
		vector last_accel, last_gyro, last_magn;
		
		
		vector f_accel,f_gyro,f_magn;		//filltered values after applly low pass filter
		pyr accel_pyr, gyro_pyr;	
		
		vector readAccel(void);
		vector readGyroAngularRate(void);
		vector readMagn(void);
		
		/*Started up functions*/
		void accelInit(void);
		void gyroInit(void);
		void magnInit(void);
		void setBalanceState(void);
		void setZeroAccel(int rate);
		void setZeroGyro(int rate);
		void setZeroMagn(int rate);
		
		//Vector calculation
		vector deltaVector(vector before, vector after);
		vector add2Vectors(vector v1, vector v2);
		vector substract2Vectors(vector v1, vector v2);
		vector avgVector(vector v1, float avgFactor);
		vector scaleVector(vector v1, float scaleFactor);
		/* Registers */
		enum info_t 
		{
			ADXL345_DEVID          = 0x00,
			...
			BMP085_ADDRESS		   =0x77,
			BMP085_CONTL_REG	   =0XF4,
			BMP085_READ_TEMPERATURE=0x2E,
			BMP085_READ_PRESSURE   =0x34,
			
		};
		
		/* Funtions */
		int readRegister(info_t device_addr, byte reg_addr, int nbytes, byte *buffer);
		int readRegister(info_t device_addr, int nbytes, byte *buffer);
		int writeRegister(info_t device_addr, int nbytes, byte *buffer);
		int writeRegister(info_t device_addr, byte reg_addr, int nbytes, byte *buffer);
		
		/*Filters and calculations*/
		vector applyLowPass(vector _raw, vector _filtered);
		pyr calculatePYR(vector _v);
};
